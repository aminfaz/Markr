...
describe('[POST] /api/import', () => {
  ...
  describe('Invalid Test Result XML', () => {
    function assertion(res) {
      expect(res.status).toBe(400);
      expect(res.text).toBe("The structure of the request does not comply with expected structure");
    }

    it('should return 400 for not having the mcq-test-results tag', async () => {
      const res = await exec('text/xml+markr', '<root><name>my name</name></root>');
      assertion(res);
    });
  ...
  describe('Valid payload', () => {
    ...
    it('should return 200 for single new result', async () => {
      ...
      expect(JSON.stringify(testResult_Actual)).toEqual(JSON.stringify(testResult_Expected));
    });
